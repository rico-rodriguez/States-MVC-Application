...
namespace MyContactManagetIntegrationTests
{
    public class TestStatesData
    {
        private DbContextOptions<MyContactManagerDbContext> _options;
        ...
        private void SetupOptions()
        {
            _options = new DbContextOptionsBuilder<MyContactManagerDbContext>()
                .UseInMemoryDatabase(databaseName: "MyContactManagertest").Options;
        }
        ...
        {
            await using (var context = new MyContactManagerDbContext(_options))
            {
        ...
        [Theory]
        [InlineData("TestState", "TS")]
        public async Task InsertAndDeleteState(string expectedName, string expectedAbbreviation)
        {
            await using var context = new MyContactManagerDbContext(_options);
            _repository = new StatesRepository(context);

            var state = new State() { Name = expectedName, Abbreviation = expectedAbbreviation };
            context.States.Add(state);
            await context.SaveChangesAsync();
            context.States.ShouldContain(state);
            state.Id.ShouldBeGreaterThan(0);

            await _repository.DeleteAsync(state);
            var existingState = await _repository.GetAsync(state.Id);
            await context.SaveChangesAsync();
            existingState.ShouldBeNull();
        }
        ...
        public async Task UpdateState()
        {
            await using var context = new MyContactManagerDbContext(_options);
            {
                ...
                var state = await _repository.GetAsync(16);
                state.Name = IOWA_SPELLING;
                await _repository.AddorUpdateAsync(state);
                await context.SaveChangesAsync();
                var updatedState = await _repository.GetAsync(16);
                updatedState.Name.ShouldBe(IOWA_SPELLING);
            }
        }
        ...
        public async Task TestExists(int stateId, bool expected)
        {
            await using var context = new MyContactManagerDbContext(_options);
            {
                _repository = new StatesRepository(context);

                var exists = await _repository.ExistsAsync(stateId);
